...
In \autoref{fig:speed_order_gausslegendre}, we show the evolution of the time required to compute the occultation light curve of a limb darkened star and that of a non-uniform star described by spherical harmonics with a maximum degrees $l_{max}=20$, depending on the order $q$ of the numerical integration. On CPU, our benchmark is done on the single core of Apple M2 max chip, while on GPU we use the single core of an NVIDIA Tesla V100 processor.\\\\
\begin{figure}[H]
    \begin{center}
...
Finally, \autoref{fig:speed_N} shows the evaluation time of \textsf{jaxoplanet} as a function of the number of points in the light curve, compared to \textsf{starry} for a surface map of degree $l_{max} = 20$, and \textsf{exoplanet} for a quadratically limb darkened star. As in the previous figure, we show that \textsf{jaxoplanet} is competitive against state-of-the-art codes on CPUs and offers a clear advantage on GPUs for large datasets.
